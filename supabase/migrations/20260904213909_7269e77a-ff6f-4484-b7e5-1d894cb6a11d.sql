CREATE OR REPLACE FUNCTION public.save_mgmv_agreement_atomic(_client_id uuid, _agreement jsonb, _installments jsonb, _client_mgmv jsonb, _product_ids uuid[] DEFAULT NULL, _restart boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
DECLARE
  v_uid uuid := auth.uid(); v_env public.app_env; v_owner uuid;
  v_existing_completed timestamptz; v_installment jsonb;
  v_numbers integer[] := ARRAY[]::integer[]; v_count integer := 0;
  v_linked integer := 0; v_restarted boolean := false;
BEGIN
  ...
  IF _agreement IS NULL OR _installments IS NULL OR jsonb_typeof(_installments) <> 'array' THEN
    ...
    UPDATE public.clients SET mgmv=NULL, client_type='common', updated_at=now() WHERE id=_client_id;
  ...
  IF v_existing_completed IS NOT NULL AND _restart THEN
    DELETE FROM public.mgmv_installments WHERE agreement_id=_client_id AND env=v_env AND (v_env <> 'sandbox'::public.app_env OR sandbox_owner=v_owner);
    UPDATE public.products SET mgmv_agreement_id=NULL, included_in_mgmv=false, collection_eligible=true, updated_at=now() WHERE mgmv_agreement_id=_client_id;
    DELETE FROM public.mgmv_agreements WHERE id=_client_id AND env=v_env AND (v_env <> 'sandbox'::public.app_env OR sandbox_owner=v_owner);
    UPDATE public.clients SET mgmv=NULL, updated_at=now() WHERE id=_client_id;
  ...
  IF _product_ids IS NULL THEN
    ...
    SELECT count(*) INTO v_linked FROM public.products WHERE client_id=_client_id AND env=v_env AND (v_env <> 'sandbox'::public.app_env OR sandbox_owner=v_owner) AND included_in_mgmv;
  ELSE
    UPDATE public.products
       SET financial_status='MGMV', included_in_mgmv=true, mgmv_agreement_id=_client_id, collection_eligible=false, updated_at=now()
     WHERE client_id=_client_id AND env=v_env AND (v_env <> 'sandbox'::public.app_env OR sandbox_owner=v_owner) AND id = ANY(_product_ids);
    GET DIAGNOSTICS v_linked = ROW_COUNT;
    UPDATE public.products
       SET financial_status=CASE
             WHEN financial_status='MGMV' AND paid_value >= total_value THEN 'Pago'
             WHEN financial_status='MGMV' THEN 'Pendente'
             ELSE financial_status END,
           included_in_mgmv=false, mgmv_agreement_id=NULL, collection_eligible=true, updated_at=now()
     WHERE client_id=_client_id AND env=v_env AND (v_env <> 'sandbox'::public.app_env OR sandbox_owner=v_owner) AND NOT (id = ANY(_product_ids));
  END IF;

  UPDATE public.clients SET mgmv=_client_mgmv,client_type=CASE WHEN COALESCE(NULLIF(_agreement->>'completed_at','')::timestamptz,v_existing_completed) IS NULL THEN 'mgmv' ELSE 'common' END,updated_at=now() WHERE id=_client_id;
  RETURN jsonb_build_object('ok',true,'installments',v_count,'linked_products',v_linked,'restarted',v_restarted);
END;
$function$;

REVOKE ALL ON FUNCTION public.save_mgmv_agreement_atomic(uuid, jsonb, jsonb, jsonb, uuid[], boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.save_mgmv_agreement_atomic(uuid, jsonb, jsonb, jsonb, uuid[], boolean) TO authenticated, service_role;