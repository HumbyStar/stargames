DROP FUNCTION IF EXISTS public.save_mgmv_agreement_atomic(uuid, jsonb, jsonb, jsonb);

CREATE OR REPLACE FUNCTION public.save_mgmv_agreement_atomic(_client_id uuid, _agreement jsonb, _installments jsonb, _client_mgmv jsonb, _product_ids uuid[] DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  RETURN jsonb_build_object('ok',true,'installments',v_count,'linked_products',v_linked);
END;
$function$;

REVOKE ALL ON FUNCTION public.save_mgmv_agreement_atomic(uuid, jsonb, jsonb, jsonb, uuid[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.save_mgmv_agreement_atomic(uuid, jsonb, jsonb, jsonb, uuid[]) TO authenticated, service_role;

-- Reparo: produtos marcados como MGMV sem vínculo com o acordo do cliente
UPDATE public.products p
   SET included_in_mgmv=true, mgmv_agreement_id=a.id, collection_eligible=false, updated_at=now()
  FROM public.mgmv_agreements a
 WHERE a.id = p.client_id AND a.env = p.env
   AND p.financial_status='MGMV' AND (p.included_in_mgmv = false OR p.mgmv_agreement_id IS NULL);

UPDATE public.products
   SET included_in_mgmv=false, mgmv_agreement_id=NULL, collection_eligible=true, updated_at=now()
 WHERE included_in_mgmv = true
   AND NOT EXISTS (SELECT 1 FROM public.mgmv_agreements a WHERE a.id = products.client_id AND a.env = products.env);