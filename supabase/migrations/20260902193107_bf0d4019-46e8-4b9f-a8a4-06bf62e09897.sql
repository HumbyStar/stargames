CREATE OR REPLACE FUNCTION public.segment_clients(_category_id uuid DEFAULT NULL::uuid, _platform text DEFAULT NULL::text, _min numeric DEFAULT 0, _max numeric DEFAULT NULL::numeric, _basis text DEFAULT 'total'::text, _exclude_situations text[] DEFAULT ARRAY[]::text[], _sort text DEFAULT 'value_desc'::text, _page integer DEFAULT 0, _page_size integer DEFAULT 20)
 RETURNS TABLE(client_id uuid, client_name text, phone text, customer_data text, products_count bigint, spent numeric, total_count bigint, group_total numeric)
 LANGUAGE plpgsql
 STABLE
...
BEGIN
  ...
  filtered AS (
    SELECT a.cid, a.cname, a.cphone, a.cdata, a.n, a.spent_v FROM agg a
